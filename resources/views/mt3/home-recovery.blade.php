                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select x-model="province" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 transition-shadow" required>
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="phone" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-200 transition-shadow" required placeholder="08X-XXX-XXXX">
                                </div>
                                <div>
...
function homeRecoveryForm() {
    return {
        lat: '', lng: '', address: '', province: '', phone: '', landmark: '', details: '', needs: [],
                ...
                (err) => { 
                    ...
                    switch(err.code) {
                        case err.PERMISSION_DENIED:
                            errorDetail = "คุณไม่อนุญาตให้เบราว์เซอร์เข้าถึงตำแหน่งที่ตั้ง";
                            break;
                        case err.POSITION_UNAVAILABLE:
                            errorDetail = "อุปกรณ์ไม่สามารถระบุพิกัดได้ (ตรวจสอบว่าเปิด GPS หรือไม่)";
                            break;
                        case err.TIMEOUT:
                            errorDetail = "หมดเวลาในการค้นหาสัญญาณ GPS";
                            break;
                        default:
                            errorDetail = err.message;
                            break;
                    }
                    this.errorMessage = "ไม่สามารถดึงตำแหน่ง GPS อัตโนมัติ: " + errorDetail + " กรุณากดที่แผนที่เพื่อเลือกตำแหน่งด้วยตัวเองครับ";
                    this.showErrorModal = true;
                },
        ...
        async submitRecoveryRequest() {
            ...
            try {
                let response = await fetch('{{ route('mt3.home-recovery.store') }}', {
                    ...
                    body: JSON.stringify({ 
                        lat: this.lat, lng: this.lng, address: this.address, zipCode: this.province, phone: this.phone, landmark: this.landmark, needs: this.needs, details: this.details 
                    })
                });
