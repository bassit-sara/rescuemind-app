...
@section('page-title', '✏️ แก้ไขแจ้งเตือน')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="level" required
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        <option value="1" {{ old('level', $alert->level) == 1 ? 'selected' : '' }}>🟡 ระดับ 1 — เฝ้าระวัง</option>
                        <option value="2" {{ old('level', $alert->level) == 2 ? 'selected' : '' }}>🟠 ระดับ 2 — เตรียมอพยพ</option>
                        <option value="3" {{ old('level', $alert->level) == 3 ? 'selected' : '' }}>🔴 ระดับ 3 — อพยพทันที</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="is_active"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        <option value="1" {{ old('is_active', $alert->is_active) ? 'selected' : '' }}>✅ เปิดใช้งาน</option>
                        <option value="0" {{ !old('is_active', $alert->is_active) ? 'selected' : '' }}>❌ ปิดใช้งาน</option>
                    </select>
                </div>
            </div>
