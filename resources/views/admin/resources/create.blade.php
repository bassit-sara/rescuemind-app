...
@section('page-title', '📦 เพิ่มทรัพยากรช่วยเหลือภัยพิบัติ')
@section('content')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="boat" {{ old('type')=='boat'?'selected':'' }}>⛵ เรือ</option>
                        <option value="truck" {{ old('type')=='truck'?'selected':'' }}>🚚 รถยนต์ / รถบรรทุก</option>
                        <option value="medicine" {{ old('type')=='medicine'?'selected':'' }}>💊 ยา / เวชภัณฑ์</option>
                        <option value="food" {{ old('type')=='food'?'selected':'' }}>🍱 อาหารแห้ง / ถุงยังชีพ</option>
                        <option value="water" {{ old('type')=='water'?'selected':'' }}>💧 น้ำดื่ม</option>
