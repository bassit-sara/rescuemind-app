...
<div x-data="{ 
    showModal: false, 
    modalMode: 'add', 
    currentItem: { id: null, name: '', phone: '', skills: '', area: '' },
    items: [
        { id: '#VL-001', name: 'นพ. ภพธร', phone: '082-xxx-xxxx', skills: 'แพทย์อาสา, ปฐมพยาบาล', area: 'เชียงราย' },
        { id: '#VL-002', name: 'ช่างดำ แข็งขัน', phone: '086-xxx-xxxx', skills: 'ช่างไฟฟ้า, ช่างยนต์', area: 'เชียงใหม่' }
    ],
    init() {
        const stored = localStorage.getItem('mt3_volunteer');
        if (stored) {
            this.items = JSON.parse(stored);
        }
        this.$watch('items', value => {
            localStorage.setItem('mt3_volunteer', JSON.stringify(value));
        });
    },
    saveItem() {
        if(this.modalMode === 'add') {
            this.currentItem.id = '#VL-' + String(this.items.length + 1).padStart(3, '0');
        ...
        Swal.fire({
            ...
            html: `
                <div class='text-left space-y-2 mt-4 text-sm text-gray-700 bg-gray-50 p-4 rounded-xl border border-gray-100'>
                    ...
                    <p><span class='font-bold text-gray-900'>ชื่ออาสาสมัคร:</span> ${item.name}</p>
                    <p><span class='font-bold text-gray-900'>เบอร์ติดต่อ:</span> ${item.phone}</p>
                    <p><span class='font-bold text-gray-900'>ทักษะ/ความเชี่ยวชาญ:</span> ${item.skills}</p>
                    <p><span class='font-bold text-gray-900'>พื้นที่ปฏิบัติงาน:</span> ${item.area}</p>
                </div>
            `,
    ...
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
        ...
        <div class="flex gap-2">
            <button @click="showModal = true; modalMode = 'add'; currentItem = { id: null, name: '', phone: '', skills: '', area: '' }" class="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-sm hover:bg-indigo-700 flex items-center gap-2 transition-colors">
            ...
            <tbody class="divide-y divide-gray-100">
                <template x-for="item in items" :key="item.id">
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600" x-text="item.skills"></td>
                        <td class="px-6 py-4 text-sm text-gray-600" x-text="item.area"></td>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title" x-text="modalMode === 'add' ? 'เพิ่มอาสาสมัคร' : 'แก้ไขข้อมูลอาสาสมัคร'"></h3>
                                <div class="mt-4 space-y-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">ชื่ออาสาสมัคร</label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">ทักษะ/ความเชี่ยวชาญ</label>
                                        <input type="text" x-model="currentItem.skills" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">พื้นที่ปฏิบัติงาน</label>
                                        <input type="text" x-model="currentItem.area" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>
                                </div>
                            </div>
                        </div>
