...
@section('page-title', $news ? '✏️ แก้ไขข่าวสาร' : '➕ เพิ่มข่าวสาร')
        ...
        <form action="{{ $news ? route('admin.news.update', $news) : route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <select name="category" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-300">
                    @foreach(['general' => '📰 ทั่วไป', 'disaster' => '🚨 ภัยพิบัติ', 'alert' => '⚠️ แจ้งเตือน', 'health' => '🏥 สุขภาพ', 'community' => '👥 ชุมชน'] as $val => $label)
            ...
            <div>
                ...
                @if($news && $news->image_url)
                    <div class="mt-3 relative inline-block">
                        ...
                        <span class="absolute top-2 right-2 bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded shadow">📸 รูปปัจจุบัน</span>
                    </div>
                @endif
                ...
                @if($news && $news->video_url)
                    <div class="mt-3 relative inline-block">
                        ...
                        <span class="absolute top-2 right-2 bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded shadow">🎥 วิดีโอปัจจุบัน</span>
                    </div>
                @endif
            ...
            <div class="flex flex-wrap gap-6 pt-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-sm font-medium text-gray-700">📌 ปักหมุดที่ด้านบน</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <div class="relative">
                    ...
                    <span class="text-sm font-medium text-gray-700">✅ เผยแพร่ทันที</span>
                </label>
            </div>
            ...
            <div class="flex gap-3 pt-2 border-t border-gray-100">
                <button type="submit" class="px-6 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition shadow-sm">
                    {{ $news ? '💾 บันทึกการแก้ไข' : '📤 เผยแพร่ข่าวสาร' }}
                </button>
